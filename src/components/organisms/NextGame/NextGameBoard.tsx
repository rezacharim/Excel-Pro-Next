...
const NextGameBoard = ({
  ...
  // The card needs an upcoming game, but results are worth showing on their
  // own at the end of a season when there is nothing left to play.
  ...
  return (
    <section className="mx-4 my-16 sm:my-20">
      <div className="mx-auto max-w-7xl">
        ...
        {results.length > 0 && (
          <div className="mt-10">
            <h3 className="mb-4 text-xl font-bold text-[#020022]">
              Recent results
            </h3>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {results.slice(0, 4).map((f) => {
                const ours = f.ourScore ?? 0;
                const theirs = f.theirScore ?? 0;
                const known = f.ourScore != null && f.theirScore != null;
                const outcome = !known
                  ? { label: "Played", tone: "bg-gray-100 text-gray-600" }
                  : ours > theirs
                    ? { label: "Won", tone: "bg-green-50 text-green-700" }
                    : ours < theirs
                      ? { label: "Lost", tone: "bg-red-50 text-red-700" }
                      : { label: "Drew", tone: "bg-amber-50 text-amber-700" };
                return (
                  <div
                    key={f.id}
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[11px] font-extrabold text-gray-600">
                        {f.ageGroup}
                      </span>
                      <span
                        className={`rounded-full px-2 py-0.5 text-[10px] font-extrabold ${outcome.tone}`}
                      >
                        {outcome.label}
                      </span>
                    </div>
                    <p className="text-2xl font-extrabold leading-none text-[#020022]">
                      {known ? `${ours}\u2013${theirs}` : "\u2013"}
                    </p>
                    <p className="mt-1.5 text-sm font-semibold text-[#020022]">
                      <span className="font-normal text-gray-400">
                        {f.isHome ? "vs" : "at"}
                      </span>{" "}
                      {f.opponent}
